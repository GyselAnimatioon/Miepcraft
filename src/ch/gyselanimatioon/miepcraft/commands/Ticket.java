	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (player != null) {
				...
				if (args[0].equalsIgnoreCase("add")) {
					...
					if (!sender.hasPermission("miepcraft.commands.ticket.list")) {
						...
						for (int i = 0; i < list.size(); i++) {
							sender.sendMessage("§8[§eTicket§8] §6" + (String) list.get(i));
						}
					}
				} else {
					player.sendMessage("§8[§eTicket§8] §6/ticket add <Ticket>");
			...
			if (args[0].equalsIgnoreCase("add")) {
				...
				List<String> list = PluginMain.fileManager.read(".", "tickets");
				for (int i = 0; i < list.size(); i++) {
					Bukkit.getConsoleSender().sendMessage("§8[§eTicket§8] §6" + (String) list.get(i));
				}
			} else {
				Bukkit.getConsoleSender().sendMessage("§8[§eTicket§8] §6/ticket add <Ticket>");
